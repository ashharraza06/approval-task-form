sap.ui.define(
  ...
  function (BaseController) {
    ...
    return BaseController.extend("form.workflowuimodule.controller.App", {
      async onInit() {
        debugger
        // Function to wait for context data to become available
        const waitForContextData = async (maxRetries = 10, delayMs = 500) => {
          let retries = 0;
          let contextData = null;

          while (retries < maxRetries) {
            contextData = this.getOwnerComponent().oModels.context?.oData;

            if (contextData && Object.keys(contextData).length > 0) {
              return contextData; // Return valid contextData
            }
            await new Promise(resolve => setTimeout(resolve, delayMs));
            retries++;
          }

          throw new Error("Failed to load context data after multiple attempts.");
        };


        try {
          // aWait for context data to become available
          contextData = await waitForContextData();
          console.log("Context data loaded successfully:", contextData);
        } catch (error) {
          console.error(error.message);
          return; // Exit if context data is not available
        }
        ...
        //   return;
        // }
        //******************** Retrieve the UserInfo service asynchronously******************************************************************************     
        try {
          // username = new sap.ushell.services.UserInfo().getEmail();
          const oContainer = sap.ushell.Container;
          if (oContainer) {
            const oUserInfo = await oContainer.getServiceAsync("UserInfo");
            username = oUserInfo.getEmail(); // Retrieve the email of the logged-in user
            console.log("User Email:", username);
          } else {
            console.error("sap.ushell.Container is not available. Are you running in a Fiori Launchpad?");
            username = "ashhar.raza@peolsolutions.com"
          }
        } catch (error) {
      ...
      //   this.byId("appid").setText(username);
      //   // username = new sap.ushell.services.UserInfo().getEmail();
      //   username = "ashhar.raza@peolosolutions.com"
      //   compno = this.getOwnerComponent().oModels.context.oData.complainno;
      //   clevel = this.getOwnerComponent().oModels.context.oData.clevel;
      //   var mainService = this.getOwnerComponent().oModels.context.oData.baseurl;
      //   mainServiceUrl = "https://" + mainService;
      ...
      onAfterItemAdded: function (oEvent) {
        ...
        _createEntity(item)
          .then((id) => {
            ...
            // // Update the model with the new file data
            // const oModel = this.getView().getModel("context");
            // const files = oModel.getProperty("/filesData") || [];
            // files.push(newFileData);
            // oModel.setProperty("/filesData", files);
